...
\lstinputlisting[label=code:initUART, language=C, firstline=351, lastline=365, tabsize=2, caption=Κώδικας αρχικοποίησης UART]{main.c}

\subsection{Επεξεργασία των εντολών}
Οι εντολές που υποστηρίζει το ενσωματώμενο σύστημα παρατίθενται στο Table~\ref{table:commands}. Κατά το χρόνο σύνταξης της παρούσας αναφοράς, οι εντολές εκτελούνται στο Timer 2 interrupt. Συγκεκριμένα, κάθε φορά που εκτελείται το Timer 2 interrupt, διαβάζεται ο receive buffer και αποκωδικοποείται μία νέα εντολή, εφόσον υπάρχει. Η εκτέλεση των εντολών, γίνεται και αυτή στο timer interrupt το οποίο για εντολές με μεγάλο χρόνο εκτέλεσης, όπως το "play new game" 

%έχει πλεονεκτήματα και μειονεκτήματα. Αρχικά, ο έλεγχος που δίνει ο timer είναι ένα σημαντικό πλεονέκτημα, εφόσον μπορεί να ελεχθεί η περιοδικότητα εκτέλεσης των εντολών σε σχέση με τη λήψη και μετάδοση δεδομένων.
